...
AddEventHandler('esx:setJob', function(job)
	PlayerData.job = job
	if job.name == Config.Insurance.job.name then
		JobUpdateBrokerCount()
	end
end)

-- [[ ESX SHOW ADVANCED NOTIFICATION ]] --
RegisterNetEvent('t1ger_insurance:ShowAdvancedNotifyESX')
AddEventHandler('t1ger_insurance:ShowAdvancedNotifyESX', function(title, subject, msg, icon, iconType)
...
RegisterNetEvent('t1ger_insurance:ShowNotifyESX')
AddEventHandler('t1ger_insurance:ShowNotifyESX', function(msg)
	ShowNotifyESX(msg)
end)
